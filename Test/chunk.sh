...
function run_tests() {
    ...
    time_dir="$output_dir/${chunk_size}"
...
do
    ...
    run_tests "$step_size" "$chunks" "$num_ranks" "$block_size" "$output_dir"/FixStepSmallCompute
    ...
    run_tests "$step_size" "$chunks" "$num_ranks" "$block_size" "$output_dir"/FixStepLargeCompute
    ...
    run_tests "$step_size" "$chunks" "$num_ranks" "$block_size" "$output_dir"/FixComputeLargeStep
    ...
    run_tests "$step_size" "$chunks" "$num_ranks" "$block_size" "$output_dir"/FixComputeSmallStep
